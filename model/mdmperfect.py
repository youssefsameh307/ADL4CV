...
class MDM(nn.Module):
    ...
    def forward(self, x, timesteps, y=None ,img_condition=None):
        ...
        img_condition = torch.stack(conditions)
        img_condition = img_condition.reshape(-1,3,480,480)
        
        # im1,im2,im3 = img_condition[0:3]
        # f1 = int(img_condition[:,3] * nframes)
        # f2 = int(img_condition[:,4] * nframes)
        # f3 = int(img_condition[:,5] * nframes)
        
        # p1,p2,p3 = create_quadratic_pattern(index=f1,length=nframes),create_quadratic_pattern(index=f2,length=nframes),create_quadratic_pattern(index=f3,length=nframes)
        
        
        
        # TODO add conditioning here 
        # img_condition = torch.stack(img_condition[0:3])
        ...
        img_embed = img_embed.repeat(1, 1, 1, nframes) #bs*3,263,1,196
        weights = weights.to('cuda')
        weights = weights.unsqueeze(1).unsqueeze(1)
        weights = weights.repeat(1,img_embed.shape[1],1,1)
        
        img_embed = img_embed * weights
        reshaped_tensor = img_embed.view(bs, 3, njoints, 1, nframes)
        img_embed = torch.sum(reshaped_tensor, dim=1)
